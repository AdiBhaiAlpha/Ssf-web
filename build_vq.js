...
// First let's write out the new VQ and GQ implementations to a clean module
const newCode = `
// ==================== ENHANCED VQ CLASS ====================
class VQ {
  ...
  static async drawLayers(ctx, width, height, item, options, theme, leftLogo, rightLogo, featuredImage, metrics) {
    ...
    const registerCoord = (name, x, y, w, h) => {
      if (metrics) {
        ...
        metrics.objectCoordinates[name] = { 
    ...
    if (options.showWatermark && leftLogo) {
      ...
      const wmSize = Math.min(width, height) * 0.48;
...
fs.writeFileSync('vq_replacement.js', newCode, 'utf8');
console.log('Saved vq_replacement.js');
