...
// 1. Update VQ getOff
const oldGetOff = `const getOff = (key) => {
      const o = (options.offsets && options.offsets[key]) || { x: 0, y: 0 };
      return { x: (o.x || 0) * scale, y: (o.y || 0) * scale };
    };`;

const newGetOff = `const getOff = (key) => {
      const o = (options.offsets && options.offsets[key]) || { x: 0, y: 0 };
      return { x: Number(o.x || 0), y: Number(o.y || 0) };
    };`;

if (bundle.includes(oldGetOff)) {
  bundle = bundle.replace(oldGetOff, newGetOff);
  console.log('Successfully updated getOff in VQ');
} else {
  console.log('getOff format might differ, checking regex...');
  const getOffRegex = /const\s+getOff\s*=\s*\(\s*key\s*\)\s*=>\s*\{[\s\S]*?return\s*\{[\s\S]*?\};\s*\};/;
  if (getOffRegex.test(bundle)) {
    bundle = bundle.replace(getOffRegex, newGetOff);
    console.log('Successfully updated getOff in VQ via regex');
  } else {
    console.warn('Could not match getOff regex in bundle');
  }
}
...
const gqStart = bundle.indexOf('function GQ({', zsStart);
const wqStart = bundle.indexOf('function WQ(', gqStart);

if (zsStart !== -1 && gqStart !== -1 && wqStart !== -1) {
  const gqCode = fs.readFileSync('upgraded_gq.js', 'utf8');
  bundle = bundle.slice(0, zsStart) + all28Templates + '\n' + gqCode + '\n' + bundle.slice(wqStart);
  fs.writeFileSync(bundlePath, bundle, 'utf8');
  console.log('Successfully patched assets/index-DkKEx6Oj.js!');
} else {
  console.error('Could not find boundaries in bundle!');
}
