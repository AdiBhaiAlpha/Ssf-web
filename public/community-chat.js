...
(function() {
  ...
  async function fetchMessages() {
    ...
    try {
      ...
      // Graceful background retry without unhandled exception log
    } finally {
      isFetching = false;
    }
  }
                ...
                return `
                  <div class="ssf-msg-card ${isMine ? "mine" : "theirs"}">
                    <div style="display: flex; align-items: center; gap: 6px; margin-bottom: 2px; font-size: 11px; color: #71717a; padding: 0 4px;">
                      <span style="font-weight: 600; color: #3f3f46;">${escapeHtml(m.userName)}</span>
                      <span>•</span>
                      <span>${timeStr}</span>
                        ...
                        <div style="margin-top: 6px;">
                          ${(m.attachment.type === "image" || (m.attachment.url && m.attachment.url.match(/\.(png|jpe?g|gif|webp|bmp|svg)($|\?)/i)) || (m.attachment.url && m.attachment.url.includes("ibb.co"))) ? `
                            <div style="position: relative; overflow: hidden; border-radius: 8px; border: 1px solid rgba(0,0,0,0.08); background: #18181b;">
                              <a href="${escapeHtml(m.attachment.url)}" target="_blank" rel="noopener noreferrer" style="display: block;">
                                <img src="${escapeHtml(m.attachment.url)}" alt="${escapeHtml(m.attachment.name || 'Attachment')}" loading="lazy" style="width: 100%; max-height: 220px; object-fit: cover; display: block; cursor: pointer; transition: transform 0.2s;" onmouseover="this.style.transform='scale(1.02)'" onmouseout="this.style.transform='scale(1)'" />
                              </a>
                              <div style="padding: 4px 8px; background: rgba(0,0,0,0.7); color: #fff; font-size: 10px; display: flex; justify-content: space-between; align-items: center;">
                                <span style="overflow: hidden; text-overflow: ellipsis; white-space: nowrap; max-width: 180px;">${escapeHtml(m.attachment.name || 'ছবি')}</span>
                                <a href="${escapeHtml(m.attachment.url)}" target="_blank" rel="noopener noreferrer" style="color: #fda4af; text-decoration: none; font-weight: bold; margin-left: 6px;">সম্পূর্ণ দেখুন ↗</a>
                              </div>
                            </div>
                          ` : `
                            <a href="${escapeHtml(m.attachment.url)}" target="_blank" rel="noopener noreferrer" style="display: flex; align-items: center; gap: 6px; background: rgba(0,0,0,0.06); padding: 6px 10px; border-radius: 6px; color: inherit; font-size: 12px; text-decoration: none;">
