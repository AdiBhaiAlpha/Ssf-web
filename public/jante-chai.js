(function() {
  ...
  let searchQuery = '';
  let activeTab = 'home'; // 'home' or 'qa-list' or 'qa-detail'
  ...
  function getUserEmail() {
    // Try to get logged in user email from localStorage or global app state
  ...
  function renderJanteChai() {
    injectStyles();
    
    // 1. Inject Nav Link if not present
    const navContainer = document.querySelector('nav') || document.querySelector('header');
    if (navContainer && !document.getElementById('nav-btn-qa')) {
      // Find where nav links are located or append to header buttons
      const headerFlex = document.querySelector('header .flex') || navContainer;
      if (headerFlex) {
        const qaNavBtn = document.createElement('button');
        qaNavBtn.id = 'nav-btn-qa';
        qaNavBtn.className = 'px-3 py-1.5 text-xs md:text-sm font-bold uppercase tracking-wider text-zinc-700 dark:text-zinc-200 hover:text-rose-600 dark:hover:text-rose-400 transition-colors flex items-center gap-1.5 cursor-pointer';
        qaNavBtn.innerHTML = `<span>❓</span><span>জানতে চাই</span>`;
        qaNavBtn.onclick = () => {
          activeTab = 'qa-list';
          renderView();
        };
        // Insert before last element or append
        headerFlex.appendChild(qaNavBtn);
      }
    }

    // 2. Check if user is viewing home or qa section
    // We can observe URL hash or custom router state
    const urlParams = new URLSearchParams(window.location.search);
    if (urlParams.get('tab') === 'qa' && activeTab === 'home') {
      activeTab = 'qa-list';
    }
  ...
  function renderView() {
    let mainContainer = document.getElementById('jante-chai-app-root');
    if (!mainContainer) {
      mainContainer = document.createElement('div');
      mainContainer.id = 'jante-chai-app-root';
      const root = document.getElementById('root');
      if (root && root.nextSibling) {
        root.parentNode.insertBefore(mainContainer, root.nextSibling);
      } else if (root) {
    ...
    // Also check homepage section insertion below Events section
    const eventsSection = document.querySelector('#events-section') || document.querySelector('.events-section') || document.querySelector('section:nth-of-type(3)') || document.querySelector('main > div > div:nth-child(3)');
    
    // Render Q&A Home Preview Section if on main home view and activeTab === 'home'
    let homePreviewHTML = '';
    ...
    const html = `
      <div class="jc-container max-w-7xl mx-auto px-4 py-8">
        ${activeTab === 'home' ? `
          <!-- Homepage Q&A Section (Below Events) -->
          <div class="my-12 p-6 bg-zinc-50 dark:bg-zinc-900/50 rounded-2xl border border-zinc-200 dark:border-zinc-800 shadow-sm">
            <div class="flex flex-col md:flex-row md:items-end justify-between mb-8 gap-4">
              ...
              <div class="flex items-center gap-3">
                ...
                <button onclick="window.JC_setTab('qa-list')" class="px-4 py-2 bg-zinc-200 dark:bg-zinc-800 hover:bg-zinc-300 dark:hover:bg-zinc-700 text-zinc-800 dark:text-zinc-200 text-xs font-bold rounded-lg transition cursor-pointer">
                  সব প্রশ্ন দেখুন →
                </button>
              </div>
            </div>
          ...
          <div class="py-6 space-y-8 animate-fadeIn">
            ...
            <div class="bg-gradient-to-r from-rose-900 via-rose-800 to-zinc-900 text-white rounded-2xl p-8 md:p-12 shadow-xl relative overflow-hidden">
              ...
              <div class="relative z-10 max-w-3xl">
                ...
                <p class="text-base md:text-lg text-rose-100 font-normal leading-relaxed mb-6">
                  রাজনীতি, সমাজ, শিক্ষা ও সমসাময়িক বিষয় নিয়ে প্রশ্ন করুন, মতামত জানুন এবং গঠনমূলক আলোচনায় অংশ নিন।
                </p>
            ...
            <div class="flex flex-col md:flex-row gap-4 items-center justify-between bg-white dark:bg-zinc-900 p-4 rounded-xl border border-zinc-200 dark:border-zinc-800 shadow-sm">
              <div class="w-full md:w-96 relative">
                <input type="text" id="jc-search-input" value="${searchQuery}" oninput="window.JC_setSearch(this.value)" placeholder="প্রশ্ন খুঁজুন (শিরোনাম বা বিবরণ)..." class="w-full pl-10 pr-4 py-2.5 text-sm bg-zinc-50 dark:bg-zinc-800 border border-zinc-300 dark:border-zinc-700 rounded-lg text-zinc-900 dark:text-white focus:outline-none focus:border-rose-600">
              ...
              <div class="flex items-center gap-3 w-full md:w-auto overflow-x-auto pb-2 md:pb-0">
                ...
                <select id="jc-sort-select" onchange="window.JC_setSort(this.value)" class="px-3 py-2 text-xs bg-zinc-50 dark:bg-zinc-800 border border-zinc-300 dark:border-zinc-700 rounded-lg text-zinc-900 dark:text-white">
                  ...
                  <option value="popular" ${currentSort==='popular'?'selected':''}>জনপ্রিয় / বেশি উত্তর</option>
                </select>
                <button onclick="window.JC_setTab('home')" class="px-3 py-2 bg-zinc-200 dark:bg-zinc-800 text-zinc-800 dark:text-zinc-200 text-xs font-bold rounded-lg hover:bg-zinc-300 transition whitespace-nowrap">
                  ← হোমপেজ
                </button>
              </div>
            </div>
            ...
            <div class="flex items-center gap-2 overflow-x-auto pb-2 scrollbar-none">
              ${categories.map(cat => `
                <button onclick="window.JC_setFilter('${cat}')" class="px-4 py-2 rounded-lg text-xs font-bold whitespace-nowrap transition cursor-pointer ${currentFilter === cat ? 'bg-rose-600 text-white shadow-md' : 'bg-white dark:bg-zinc-900 text-zinc-700 dark:text-zinc-300 border border-zinc-200 dark:border-zinc-800 hover:border-rose-400'}">
                  ${cat}
                </button>
              `).join('')}
            </div>
            ...
            <div class="space-y-4">
              ${filterQuestions().length === 0 ? `
                <div class="text-center py-16 bg-white dark:bg-zinc-900 rounded-2xl border border-zinc-200 dark:border-zinc-800">
                  ...
                  <p class="text-xs text-zinc-500 mt-1 mb-6">এই ক্যাটাগরিতে কোনো প্রশ্ন পাওয়া যায়নি। প্রথম প্রশ্নটি আপনিই করুন!</p>
                  <button onclick="window.JC_openAskModal()" class="px-5 py-2.5 bg-rose-600 text-white text-xs font-bold rounded-xl hover:bg-rose-700 transition">প্রশ্ন করুন</button>
                </div>
                  ...
                  <div class="space-y-2 flex-1">
                    ...
                    ${q.tags && q.tags.length ? `
                      <div class="flex flex-wrap gap-1.5 pt-1">
                        ${q.tags.map(t => `<span class="px-2 py-0.5 text-[10px] bg-zinc-100 dark:bg-zinc-800 text-zinc-600 dark:text-zinc-400 rounded">#${t}</span>`).join('')}
                      </div>
                    ` : ''}
                  </div>
              ...
              <div class="py-6 space-y-8 animate-fadeIn max-w-4xl mx-auto">
                ...
                <div class="bg-white dark:bg-zinc-900 p-8 rounded-2xl border border-zinc-200 dark:border-zinc-800 shadow-sm space-y-4">
                  <div class="flex items-center justify-between">
                    ...
                    <div class="flex items-center gap-3 text-xs text-zinc-500">
                      ...
                      ${(isSuper || q.authorEmail === userEmail) ? `
                        <button onclick="window.JC_deleteQuestion('${q.id}')" class="text-rose-600 hover:underline font-bold">ডিলিট</button>
                      ` : ''}
                    </div>
                  </div>
                  ...
                  ${q.tags && q.tags.length ? `
                    <div class="flex flex-wrap gap-1.5 pt-4">
                      ${q.tags.map(t => `<span class="px-2.5 py-1 text-xs bg-zinc-100 dark:bg-zinc-800 text-zinc-600 dark:text-zinc-400 rounded-lg">#${t}</span>`).join('')}
                    </div>
                  ` : ''}
                </div>
                ...
                <div class="space-y-6">
                  <h3 class="text-xl font-bold text-zinc-900 dark:text-white flex items-center gap-2">
                    <span>💬</span> উত্তরসমূহ (${q.answers ? q.answers.length : 0})
                  </h3>

                  <!-- Answer Form -->
                  <div class="bg-white dark:bg-zinc-900 p-6 rounded-2xl border border-zinc-200 dark:border-zinc-800 shadow-sm space-y-4">
                    <h4 class="text-sm font-bold text-zinc-800 dark:text-zinc-200">আপনার উত্তর দিন</h4>
                    ${!userEmail ? `
                      <div class="p-4 bg-amber-50 dark:bg-amber-950/30 border border-amber-200 dark:border-amber-900 rounded-xl text-xs text-amber-800 dark:text-amber-300 flex items-center justify-between">
                        <span>উত্তর দেওয়ার জন্য আপনাকে প্রথমে পোর্টালে লগইন করতে হবে।</span>
                        <button onclick="window.JC_promptLogin()" class="px-3 py-1.5 bg-rose-600 text-white rounded-lg font-bold">লগইন করুন</button>
                      </div>
                    ` : `
                      <form onsubmit="window.JC_submitAnswer(event, '${q.id}')" class="space-y-3">
                        <textarea id="jc-answer-content" rows="4" placeholder="আপনার উত্তর বিস্তারিত লিখুন..." class="w-full p-3 text-sm bg-zinc-50 dark:bg-zinc-800 border border-zinc-300 dark:border-zinc-700 rounded-xl text-zinc-900 dark:text-white focus:outline-none focus:border-rose-600" required></textarea>
                        <div class="flex justify-end">
                          <button type="submit" class="px-6 py-2.5 bg-rose-600 hover:bg-rose-700 text-white text-xs font-bold rounded-xl shadow transition cursor-pointer">
                            উত্তর প্রকাশ করুন
                          </button>
                        </div>
                      </form>
                  ...
                  <div class="space-y-4">
                    ${!q.answers || q.answers.length === 0 ? `
                      <div class="p-8 text-center bg-white dark:bg-zinc-900 rounded-2xl border border-zinc-200 dark:border-zinc-800 text-xs text-zinc-500">
                        এই প্রশ্নে এখনও কেউ উত্তর দেয়নি। প্রথম উত্তরদাতা হোন!
                      </div>
                    ` : q.answers.map(a => `
                      <div class="bg-white dark:bg-zinc-900 p-5 rounded-2xl border border-zinc-200 dark:border-zinc-800 space-y-2">
                        <div class="flex items-center justify-between text-xs text-zinc-500">
                          <span class="font-bold text-zinc-800 dark:text-zinc-200">👤 ${a.author}</span>
                          <div class="flex items-center gap-3">
                            <span>📅 ${a.createdAt}</span>
                            ${(isSuper || a.authorEmail === userEmail) ? `
                              <button onclick="window.JC_deleteAnswer('${q.id}', '${a.id}')" class="text-rose-600 hover:underline font-bold">ডিলিট</button>
                            ` : ''}
                          </div>
                        </div>
                        <p class="text-sm text-zinc-700 dark:text-zinc-300 whitespace-pre-wrap">${a.content}</p>
                      </div>
                    `).join('')}
                  </div>
                </div>
  ...
  function filterQuestions() {
    ...
    if (currentFilter !== 'সব') {
      result = result.filter(q => q.category === currentFilter);
    }
    if (searchQuery.trim()) {
  ...
  window.JC_setFilter = function(cat) {
    currentFilter = cat;
    renderView();
  };
  ...
  window.JC_setSearch = function(val) {
    searchQuery = val;
    renderView();
  };
  ...
  window.JC_promptLogin = function() {
    alert('অনুগ্রহ করে পোর্টালে লগইন করুন। (মেম্বার অ্যাকাউন্ট বা গুগল লগইন ব্যবহার করুন)');
    // Try to trigger existing login button
    const loginBtn = document.querySelector('button[title*="লগইন"]') || document.querySelector('button');
  ...
  window.JC_openAskModal = function() {
    ...
    modal.className = 'fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-xs p-4';
    modal.innerHTML = `
      <div class="bg-white dark:bg-zinc-900 w-full max-w-xl rounded-2xl shadow-2xl border border-zinc-200 dark:border-zinc-800 p-6 md:p-8 space-y-6 animate-scaleIn">
        <div class="flex items-center justify-between pb-4 border-b border-zinc-100 dark:border-zinc-800">
          ...
          <button onclick="document.getElementById('jc-ask-modal').remove()" class="text-zinc-400 hover:text-zinc-700 dark:hover:text-white text-lg font-bold">✕</button>
        </div>
        <form onsubmit="window.JC_submitQuestion(event)" class="space-y-4">
          ...
          <div class="flex justify-end gap-3 pt-4 border-t border-zinc-100 dark:border-zinc-800">
            <button type="button" onclick="document.getElementById('jc-ask-modal').remove()" class="px-5 py-2.5 bg-zinc-200 dark:bg-zinc-800 text-zinc-800 dark:text-zinc-200 text-xs font-bold rounded-xl hover:bg-zinc-300 transition">বাতিল</button>
            <button type="submit" class="px-6 py-2.5 bg-rose-600 hover:bg-rose-700 text-white text-xs font-bold rounded-xl shadow transition">প্রশ্ন প্রকাশ করুন</button>
          </div>
        </form>
      </div>
    `;
  ...
  window.JC_submitQuestion = async function(e) {
    ...
    try {
      const res = await fetch('/api/questions', {
        ...
        body: JSON.stringify({
          question: { title, content, category, tags, author: userEmail.split('@')[0] },
          userEmail
        })
      });
      if (res.ok) {
  ...
  window.JC_deleteQuestion = async function(qId) {
    ...
    try {
      ...
      if (res.ok) {
        alert('প্রশ্নটি সফলভাবে ডিলিট করা হয়েছে।');
        activeTab = 'qa-list';
        fetchQuestions();
      } else {
  ...
  window.JC_deleteAnswer = async function(qId, aId) {
    ...
    const userEmail = getUserEmail();
    try {
      const res = await fetch(`/api/questions/${qId}/answers/${aId}`, {
        method: 'DELETE',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ userEmail })
      });
      if (res.ok) {
        alert('উত্তরটি ডিলিট করা হয়েছে।');
        fetchQuestions();
      } else {
        alert('ডিলিট করার অনুমতি নেই।');
      }
  ...
  // Initial fetch
  fetchQuestions();
  setInterval(fetchQuestions, 15000); // Poll for updates
})();
