...
(function() {
  ...
  function revokeAllBlobUrls() {
    for (var i = 0; i < _activeBlobUrls.length; i++) {
      try { URL.revokeObjectURL(_activeBlobUrls[i]); } catch(e) {}
    }
    _activeBlobUrls = [];
  }
  ...
  /* ============================================================
     EXPORT / DOWNLOAD HELPERS (fixes CORS, DPI, mobile download)
     ============================================================ */

  function exportCanvasToBlob(canvas, type, quality) {
    return new Promise(function (resolve, reject) {
      ...
      try {
        canvas.toBlob(function (blob) {
          if (!blob) {
            reject(new Error('Canvas toBlob returned null — likely tainted by cross-origin image'));
            return;
          }
          resolve(blob);
  ...
  function downloadBlob(blob, filename) {
    ...
    a.href = url;
    a.download = filename || 'photocard.png';
  ...
  function exportPhotocard(format, photoImage, logoImage, payload, settings, options) {
    options = options || {};
    var size = options.size || CARD_SIZE;
    var scale = options.scale || 1;
    var filename = options.filename || (settings.downloadPrefix || 'photocard') + '-' + (format || 'classic') + '.png';
    ...
    var drawFn = {
      'classic': drawClassicFormat,
      'classic-red': drawClassicFormat,
      'split': drawSplitFormat,
      'market': drawMarketFormat,
      'magazine': drawMagazineFormat,
      'frame': drawFrameFormat
    }[format] || drawClassicFormat;

    try {
      drawFn(ctx, photoImage, logoImage, payload, settings, size);
    } catch (e) {
      console.error('[Borbila V2] Draw error:', e);
      throw new Error('Failed to render photocard: ' + e.message);
    }

    return exportCanvasToBlob(canvas, 'image/png', 1.0).then(function (blob) {
      downloadBlob(blob, filename);
      return { blob: blob, canvas: canvas, filename: filename };
    });
  }

  function renderPreviewToCanvas(format, photoImage, logoImage, payload, settings, targetCanvas, options) {
    options = options || {};
    var size = options.size || CARD_SIZE;
    var ctx = targetCanvas.getContext('2d');
    var dpr = window.devicePixelRatio || 1;
    var displayW = options.displayWidth || size;
    var displayH = options.displayHeight || size;
    ...
    var scaleX = displayW / size;
    var scaleY = displayH / size;
    ...
    try {
      ...
      console.error('[Borbila V2] Preview render error:', e);
      ctx.fillStyle = '#ff4444';
      ctx.font = '700 24px sans-serif';
      ctx.textAlign = 'center';
      ctx.textBaseline = 'middle';
      ctx.fillText('Preview Error: ' + e.message, displayW / 2, displayH / 2);
    }
  ...
  window.borbilaDrawClassicFormat = drawClassicFormat;
  window.borbilaDrawSplitFormat = drawSplitFormat;
  window.borbilaDrawMarketFormat = drawMarketFormat;
  window.borbilaDrawMagazineFormat = drawMagazineFormat;
  window.borbilaDrawFrameFormat = drawFrameFormat;
  window.borbilaLoadImage = loadImage;
  window.borbilaNormalizeSettings = normalizeSettings;
  window.borbilaNormalizeHexColor = normalizeHexColor;
  window.borbilaGetTextFont = getTextFont;
  window.borbilaGetTitleFont = getTitleFont;
  window.borbilaGetTitleFontMedium = getTitleFontMedium;
  window.borbilaTrimTextToWidth = trimTextToWidth;
  window.borbilaWrapText = wrapText;
  window.borbilaDrawAdaptiveMultiline = drawAdaptiveMultiline;
  window.borbilaDefaultSettings = DEFAULT_SETTINGS;
  window.borbilaExportPhotocard = exportPhotocard;
  window.borbilaRenderPreview = renderPreviewToCanvas;
  window.borbilaDownloadBlob = downloadBlob;
  window.borbilaRevokeBlobUrls = revokeAllBlobUrls;
  window.borbilaCardSize = CARD_SIZE;

  console.log("[Borbila PhotoCard V2 Engine] Initialized successfully with 5 Pro Formats + Export System.");
})();
